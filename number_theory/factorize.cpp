...
std::vector<pair<uint64_t, size_t> > factorize_p(uint64_t n) {
    auto ps = factorize(n);
    vector<pair<uint64_t, size_t> > ret;
    if (ps.empty()) {
        return ret;
    }
    size_t cnt = 1;
    uint64_t last = ps.front();
...
pair<int64_t, int64_t> diophantine_solve (int64_t a, int64_t b, int64_t c) { 
    int64_t g = __gcd(a, b);
    if (c % g) {
    ...
    int64_t x0 = mod_pow(a, euler_f(b) - 1, b) * (c % b) % b;
    int64_t y0 = (c - x0 * a) / b;
